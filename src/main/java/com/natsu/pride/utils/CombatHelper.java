...
public class CombatHelper {
	...
	public static boolean attackCooldownDisabledFor(ItemStack stack) {
		Item item = stack.getItem();
		if (item instanceof AxeItem) return ServerConfig.DISABLE_AXE_ATTACK_COOLDOWN.get();
		if (item instanceof TridentItem) return ServerConfig.DISABLE_TRIDENT_ATTACK_COOLDOWN.get();
		// épée + tout le reste (léger)
		return ServerConfig.DISABLE_SWORD_ATTACK_COOLDOWN.get();
	}
