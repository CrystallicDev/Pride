<div align="center">

![logo](https://cdn.modrinth.com/data/cached_images/b9491457aab57e147da6120220cf6ee9db6c3e95.png)
# Pride

[![Forge](https://img.shields.io/badge/Loader-Forge-darkgreen)](https://minecraftforge.net/)
[![Neoforge](https://img.shields.io/badge/Loader-Neoforge-purple)](https://neoforged.net/)
[![Neoforge](https://img.shields.io/badge/Loader-Fabric-cyan)](https://fabricmc.net)
[![Cristallite](https://img.shields.io/badge/Cristallite-Mod_Collection-darkgreen)](https://modrinth.com/collection/4wMgAhzS)
![Modrinth Downloads](https://img.shields.io/modrinth/dt/cristallite-pride?logo=modrinth&label=Downloads&color=00AF5C)

[![ko-fi](https://ko-fi.com/img/githubbutton_sm.svg)](https://ko-fi.com/nqtsu91)

> This mod is a member of the **Cristallite Mod Collection**, see Pride and my other mods docs / wiki [here](https://google.com).

</div>

**Pride** (or Warrior's Pride) is a mod that aim to revert Minecraft PvP physics to pre 1.9, while still keeping the content of the Combat Update.

As a long-time PvP player, i've felt disappointed with the "Combat Update" that made Minecraft PvP and PvE feel slow and boring, by adding the attack cooldown on every weapon, and introducing a way to negate all incoming damage.

Pride reworks the knockback functions, and attack cooldowns to make swords and fishing rods from 1.8.9 / 1.7.10 integrate with the new axes and shields. It also tries to replicate the feel of old school PvP by changing the player's body rotation speed, and the buckets / sword animation.

## Availability 
|Loader|Version|
|--------|--------|
| Forge | 1.18.2, 1.19.2, 1.19.4, 1.20.1, 1.20.6 |
| Neoforge | 1.20.6, 1.21.1, 1.21.11, 26.1.2, 26.2 |
| Fabric | 1.18.2, 1.19.2, 1.19.4, 1.20.1, 1.20.6, 1.21.1, 1.21.11, 26.1.2, 26.2 |

## Usage 
Feel free to integrate Pride in any of your modpacks, or create new versions of this mod. However, any redistributed version, modified or not, must stay fully free, and keep a link to the original.
